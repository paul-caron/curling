...
public:
    ...
    Request() : curlHandle(curl_easy_init()), list(nullptr), cookieFile("cookies.txt"), cookieJar("cookies.txt") {
        if (instances++ == 0)
...
int main() {
    try {
        ...
        request.setURL("https://httpbin.org/cookies/set/session_id/123456789");
        request.send();
        cout << "\nResponse Data:\n" << request.getResponse() << endl;
